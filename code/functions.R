...
rvc_filter = function(x,GZ,sp,sites,year.start, year.end,strat.omit,hab.omit){ #Function to filter through RVC data
  ...
  for(i in 1:nrow(sp)){
    ...
    x6<- subset(x5,STRAT %notin% strat.omit) %>% subset (HABITAT_CD %notin% hab.omit)
...
reef_filter = function(R,GZ,sp,geog,year.start,year.end,strat.omit,hab.omit){ #function to trim dataframe and add in occurrence data by species
  ...
  site_subsets<-TempDat2 %>% group_by(geogr) %>% summarize(n=n_distinct(formid),hab_class=unique(hab_class)) %>% subset(n>=10 & is.na(hab_class)==F) #Calculate surveys per site
  TempDat3<- subset(TempDat2, geogr %in% site_subsets$geogr) #Trim out surveys from divers with less than 5 surveys in this region 
  ...
  TempDat4<- TempDat3 %>% subset(fish_memberid %in% surveyors_trim$fish_memberid) #Only keep well surveyed sites (n>=5)
  ...
  TempDat5<- subset(TempDat4, stratum %notin% strat.omit) %>% subset (hab_class %notin% hab.omit)
...
scaled_mv_timeseries_plot<- function(q,ts1,ts2,sp,params,path,TT,TT.rvc,yr.start,yr.end){
  
  #Extract parameters
  cuts=as.data.frame(params)[grepl('cut',colnames(params))]
  x=as.data.frame(params)[grepl('x',colnames(params))]; x=x[,-c(1:2)] #remove x0
  x1=x[,1:TT]
  x2=x[,c(TT+1):ncol(x)]
  ...
  pdf(file.path(path,paste(paste(q,sp,sep='_'),'.pdf',sep='')),width=8,height=6)
  ...
  hist(params$`Cor_t[1,2]`,xlab='',ylab='',axes=F,main='',breaks=30,col=adjustcolor('darkgray',alpha.f=0.6),border='white',xlim=c(-1,1))
...
binom_timeseries_plot<- function(i,ts1,ts2,sp,GZ,mod,params1,params2,path,TT,TT.rvc,n.iter,yr.start,yr.end){
  pdf(file.path(path,paste(paste(i,sp,mod,GZ,sep='_'),'.pdf',sep='')),width=8,height=6)
  
  #Extract parameters
  p1=as.data.frame(params1)
  p2=as.data.frame(params2)
  
  x=p1[grepl('x',colnames(params1))]; x=x[,-1] #remove x0
  a=p1$a
  x_1=apply(x,2,plogis)
  x_2=apply(x+a,2,plogis)
  
  x1=as.data.frame(params2)[grepl('x1',colnames(params2))]; 
  x1=apply(x1,2,plogis)
  x2=as.data.frame(params2)[grepl('x2',colnames(params2))];
  x2=apply(x2,2,plogis)
  
  y1.1=as.data.frame(params1)[grepl('a_yr1',colnames(params1))]; 
  y1.1=apply(y1.1,2,plogis)
  y1.2=as.data.frame(params1)[grepl('a_yr2',colnames(params1))]; 
  y1.2=apply(y1.2,2,plogis)
  
  y2.1=as.data.frame(params2)[grepl('a_yr1',colnames(params2))]; 
  y2.1=apply(y2.1,2,plogis)
  y2.2=as.data.frame(params2)[grepl('a_yr2',colnames(params2))]; 
  y2.2=apply(y2.2,2,plogis)
  
  if(mod=='model1'){ 
  x_mat<- matrix(NA,nrow=TT,ncol=6)
  x_mat[,1]=apply(x_1,2,median)
  x_mat[,2]=apply(x_1,2,quantile,0.05)
  x_mat[,3]=apply(x_1,2,quantile,0.95)
  x_mat[,4]=apply(x_2,2,median)
  x_mat[,5]=apply(x_2,2,quantile,0.05)
  x_mat[,6]=apply(x_2,2,quantile,0.95)
  }
  if(mod=='model2'){ 
  x_mat<- matrix(NA,nrow=TT,ncol=6)
  x_mat[,1]=apply(x1,2,median)
  x_mat[,2]=apply(x1,2,quantile,0.05)
  x_mat[,3]=apply(x1,2,quantile,0.95)
  x_mat[,4]=apply(x2,2,median)
  x_mat[,5]=apply(x2,2,quantile,0.05)
  x_mat[,6]=apply(x2,2,quantile,0.95)
  }
  ...
  for(i in 1:TT.rvc){
    if(mod=='model1'){
      y_mat_rvc[i,2]=median(y1.1[,i])
      y_mat_rvc[i,3]=quantile(y1.1[,i],0.05)
      y_mat_rvc[i,4]=quantile(y1.1[,i],0.95)
    }
    if(mod=='model2'){
      y_mat_rvc[i,2]=median(y2.1[,i])
      y_mat_rvc[i,3]=quantile(y2.1[,i],0.05)
      y_mat_rvc[i,4]=quantile(y2.1[,i],0.95)
    }
  }
  
  for(i in 1:TT){
    if(mod=='model1'){
    y_mat_reef[i,2]=median(y1.2[,i])
    y_mat_reef[i,3]=quantile(y1.2[,i],0.05)
    y_mat_reef[i,4]=quantile(y1.2[,i],0.95)
    }
    if(mod=='model2'){
      y_mat_reef[i,2]=median(y2.2[,i])
      y_mat_reef[i,3]=quantile(y2.2[,i],0.05)
      y_mat_reef[i,4]=quantile(y2.2[,i],0.95)
    }
  }
  y_mat<- full_join(y_mat_reef,y_mat_rvc)
  
  par(xpd=T)
  plot(y_mat$median.rvc~y_mat$year,type='n',ylim=c(min(x_mat),max(c(max(y_mat[,2]),max(x_mat)))),col='darkblue',bty='l',ylab=expression('Mean sighting frequency'),xlab='Year',main=paste(sp,GZ,sep=' - '))
